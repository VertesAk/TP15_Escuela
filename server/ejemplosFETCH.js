...
export async function obtenerAlumnos() {
  try {
    const alumnos = await fetch("http://192.168.1.86:8080/");
...
export async function obtenerAlumnoPorId(id) {
  try {
    const alumno = await fetch(`http://192.168.1.86:8080/alumno/${id}`);
...
export async function marcarLlamadoComoAtendido(Id_alumno) {
